...
class TurboSHAKE(object):
	...
	def __init__(self, capacity, domain_separation, data):
		...
		if result:
			raise ValueError(
				"Error %d while instantiating TurboSHAKE" % result,
			)
	...
	def update(self, data):
		...
		if result:
			raise ValueError(
				"Error %d while updating TurboSHAKE state" % result,
			)
		return self
	...
	def read(self, length):
		...
		if result:
			raise ValueError(
				"Error %d while extracting from TurboSHAKE" % result,
			)
	...
	def _reset(self):
		...
		if result:
			raise ValueError(
				"Error %d while resetting TurboSHAKE state" % result,
			)
		self._is_squeezing = False
...
def new(**kwargs):
	...
	if not (0x01 <= domain_separation <= 0x7F):
		raise ValueError(
			"Incorrect domain separation value (%d)" % domain_separation,
		)
