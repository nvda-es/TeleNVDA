...
class DsaKey(object):
	...
	def _sign(self, m, k):
		...
		blind_factor = Integer.random_range(
			min_inclusive=1,
			max_exclusive=q,
		)
...
def generate(bits, randfunc=None, domain=None):
	...
	if L != bits:
		raise ValueError(
			"Mismatch between size of modulus (%d) and 'bits' parameter (%d)" % (L, bits),
		)
	...
	if (L, N) not in [
		...
		raise ValueError(
			"Lengths of p and q (%d, %d) are not compatibleto FIPS 186-3" % (L, N),
		)
