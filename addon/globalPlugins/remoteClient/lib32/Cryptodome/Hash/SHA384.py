...
class SHA384Hash(object):
	...
	def __init__(self, data=None):
		...
		if result:
			raise ValueError(
				"Error %d while instantiating SHA384" % result,
			)
	...
	def update(self, data):
		...
		if result:
			raise ValueError(
				"Error %d while hashing data with SHA384" % result,
			)
	...
	def digest(self):
		...
		if result:
			raise ValueError(
				"Error %d while making SHA384 digest" % result,
			)
	...
	def copy(self):
		...
		clone = SHA384Hash()
		result = _raw_sha384_lib.SHA384_copy(
			self._state.get(),
			clone._state.get(),
		)
		if result:
