...
class SHA256Hash(object):
	...
	def __init__(self, data=None):
		...
		if result:
			raise ValueError(
				"Error %d while instantiating SHA256" % result,
			)
	...
	def update(self, data):
		...
		if result:
			raise ValueError(
				"Error %d while hashing data with SHA256" % result,
			)
	...
	def digest(self):
		...
		if result:
			raise ValueError(
				"Error %d while making SHA256 digest" % result,
			)
	...
	def copy(self):
		...
		clone = SHA256Hash()
		result = _raw_sha256_lib.SHA256_copy(
			self._state.get(),
			clone._state.get(),
		)
		if result:
