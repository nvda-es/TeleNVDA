...
class SHA512Hash(object):
	...
	def __init__(self, data, truncate):
		...
		if result:
			raise ValueError(
				"Error %d while instantiating SHA-512" % result,
			)
	...
	def update(self, data):
		...
		if result:
			raise ValueError(
				"Error %d while hashing data with SHA512" % result,
			)
	...
	def digest(self):
		...
		if result:
			raise ValueError(
				"Error %d while making SHA512 digest" % result,
			)
	...
	def copy(self):
		...
		result = _raw_sha512_lib.SHA512_copy(
			self._state.get(),
			clone._state.get(),
		)
		if result:
