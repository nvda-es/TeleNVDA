...
class SHA224Hash(object):
	...
	def __init__(self, data=None):
		...
		if result:
			raise ValueError(
				"Error %d while instantiating SHA224" % result,
			)
	...
	def update(self, data):
		...
		if result:
			raise ValueError(
				"Error %d while hashing data with SHA224" % result,
			)
	...
	def digest(self):
		...
		if result:
			raise ValueError(
				"Error %d while making SHA224 digest" % result,
			)
	...
	def copy(self):
		...
		clone = SHA224Hash()
		result = _raw_sha224_lib.SHA224_copy(
			self._state.get(),
			clone._state.get(),
		)
		if result:
