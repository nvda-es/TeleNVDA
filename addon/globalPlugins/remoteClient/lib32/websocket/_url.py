...
def _is_no_proxy_host(hostname: str, no_proxy: Optional[list[str]]) -> bool:
	...
	if _is_ip_address(hostname):
		return any(
			[_is_address_in_network(hostname, subnet) for subnet in no_proxy if _is_subnet_address(subnet)],
		)
...
def get_proxy_info(
	...
	if value:
		...
		auth = (unquote(proxy.username or ""), unquote(proxy.password or "")) if proxy.username else None
